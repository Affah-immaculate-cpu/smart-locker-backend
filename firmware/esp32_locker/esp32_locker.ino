...
void lcdMessage(const char* line1, const char* line2 = "") {
  ...
  snprintf(buf1, sizeof(buf1), "%s", line1);
  snprintf(buf2, sizeof(buf2), "%s", line2);

  lcd.clear();
  delay(5); // LCD controller needs ~1.5ms to finish the clear command before accepting more
  lcd.setCursor(0, 0);
  lcd.print(buf1);
  lcd.setCursor(0, 1);
  lcd.print(buf2);
...
void handleCommand(JsonDocument& doc) {
  ...
  snprintf(line1, sizeof(line1), "Locker %d", locker);

  if (strcmp(cmd, "unlock") == 0) {
    if (locker == 1 || locker == 2) {
      setLockerState(locker, true);
      lcdMessage(line1, "is open");
    }
  } else if (strcmp(cmd, "lock") == 0) {
    if (locker == 1 || locker == 2) {
      setLockerState(locker, false);
      lcdMessage(line1, "LOCKED");
    }
    ...
    if (strlen(user) > 0) {
      snprintf(line2, sizeof(line2), "%s %s", status, user);
    } else {
